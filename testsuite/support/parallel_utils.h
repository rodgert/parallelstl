...
namespace Parallel_TestUtils {

typedef double float64_t;
typedef float float32_t;

template <class T, std::size_t N>
constexpr size_t const_size(const T(&array)[N]) noexcept { return N; }

template<typename T>
class Sequence;

// Handy macros for error reporting
#define EXPECT_TRUE( condition, message ) Parallel_TestUtils::expect<true>(condition, __FILE__, __LINE__, message)
#define EXPECT_FALSE( condition, message ) Parallel_TestUtils::expect<false>(condition, __FILE__, __LINE__, message)

// Check that expected and actual are equal and have the same type.
#define EXPECT_EQ( expected, actual, message ) Parallel_TestUtils::expect_equal(expected, actual, __FILE__, __LINE__, message)

// Check that sequences started with expected and actual and have had size n are equal and have the same type.
#define EXPECT_EQ_N( expected, actual, n, message ) Parallel_TestUtils::expect_equal(expected, actual, n, __FILE__, __LINE__, message)

// Issue error message from outstr, adding a newline.
// Real purpose of this routine is to have a place to hang a breakpoint.
static void issue_error_message( std::stringstream& outstr ) {
    outstr << std::endl;
    std::cerr << outstr.str();
}
...
void expect_equal(Sequence<T>& expected, Sequence<T>& actual, const char* file, int line, const char* message) {
    ...
    if (n != m) {
        std::stringstream outstr;
        outstr << "error at " << file << ":" << line << " - "
            << message << ", expected sequence of size " << n << " got sequence of size " << m;
        issue_error_message(outstr);
        return;
    }
    ...
    for (size_t k = 0; k<n && error_count<10; ++k) {
        if (!(expected[k] == actual[k])) {
            std::stringstream outstr;
            outstr << "error at " << file << ":" << line << " - "
                << message << ", at index " << k << " expected " << expected[k] << " got " << actual[k];
            issue_error_message(outstr);
            ++error_count;
        }
    }
...
public:
    ...
    friend bool operator==( const ForwardIterator& i, const ForwardIterator& j ) {return i.my_iterator==j.my_iterator;}
    friend bool operator!=( const ForwardIterator& i, const ForwardIterator& j ) {return i.my_iterator!=j.my_iterator;}
...
struct is_equal_to {
    ...
    bool operator()( const DataType& actual ) const { return actual == m_expected; }
private:
    DataType m_expected;
};
...
inline size_t HashBits( size_t i, size_t bits ) {
...
public:
    ...
    friend bool operator==( const Number& x, const Number& y ) {return x.value == y.value;}
    friend std::ostream& operator<<( std::ostream& o, const Number& d ) {
        return o << d.value;
    }
};
...
public:
    ...
    // we need for testing.
    bool operator()( Number x, Number y ) const {
...
public:
    ...
    friend bool operator==( const MonoidElement& x, const MonoidElement& y ) {
        return x.a==y.a && x.b==y.b;
    }
    friend std::ostream& operator<<( std::ostream& o, const MonoidElement& x ) {
        return o << "[" << x.a << ".." << x.b << ")";
    }
...
public:
    ...
    MonoidElement operator()( const MonoidElement& x, const MonoidElement& y ) {
        unsigned d = 5;
...
struct BaseAdapter {
    ...
    iterator_type operator()(Iterator it) { return it; }
};
...
struct ReverseAdapter {
    ...
    iterator_type operator()(Iterator it) {
...
struct IteratorTypeAdapter<Iterator, std::forward_iterator_tag> {
    ...
    iterator_type operator()(Iterator it) { return iterator_type(it); }
};
...
struct MakeIterator {
    ...
    typename ReverseIter::iterator_type operator()(InputIterator it) {
...
struct iterator_traits_<Iter, typename std::enable_if<!std::is_void<typename Iter::iterator_category>::value,
    void>::type> {
...
struct invoke_if_ {
    ...
    void operator()(bool is_allow, Op op, Rest&&... rest) {
        if (is_allow) op(std::forward<Rest>(rest)...);
}};
template < >
...
struct iterator_invoker {
    ...
    template<typename Iterator>
    using IsConst = typename std::is_const<typename std::remove_pointer<typename std::iterator_traits<Iterator>::pointer>::type>::type;
    template<typename Iterator>
    ...
    typename std::enable_if<is_same_iterator_category<Iterator, std::random_access_iterator_tag>::value, void>::type
        ...
        invoke_if<Iterator>()(n <= sizeLimit, op, exec, make_iterator<Iterator>()(begin), n, std::forward<Rest>(rest)...);
    }
    ...
    typename std::enable_if<is_same_iterator_category<OutputIterator, std::random_access_iterator_tag>::value, void>::type
        operator()(Policy&& exec, Op op, InputIterator inputBegin, InputIterator inputEnd,
            OutputIterator outputBegin, Rest&&... rest) {
    ...
    typename std::enable_if<is_same_iterator_category<OutputIterator, std::random_access_iterator_tag>::value, void>::type
        ...
        invoke_if<InputIterator>()(std::distance(inputBegin, inputEnd) <= sizeLimit, op, exec,
            make_iterator<InputIterator>()(inputBegin), make_iterator<InputIterator>()(inputEnd),
            make_iterator<OutputIterator>()(outputBegin), make_iterator<OutputIterator>()(outputEnd), std::forward<Rest>(rest)...);
    }

    template <typename Policy, typename Op, typename InputIterator1, typename InputIterator2, typename OutputIterator, typename... Rest>
    typename std::enable_if<is_same_iterator_category<OutputIterator, std::random_access_iterator_tag>::value, void>::type
        operator()(Policy&& exec, Op op, InputIterator1 inputBegin1, InputIterator1 inputEnd1, InputIterator2 inputBegin2, InputIterator2 inputEnd2,
            OutputIterator outputBegin, OutputIterator outputEnd, Rest&&... rest) {
        invoke_if<InputIterator1>()(std::distance(inputBegin1, inputEnd1) <= sizeLimit, op, exec,
            make_iterator<InputIterator1>()(inputBegin1), make_iterator<InputIterator1>()(inputEnd1),
...
struct iterator_invoker<IteratorTag, /* IsReverse = */ std::true_type> {
    ...
    typename std::enable_if<is_same_iterator_category<OutputIterator, std::random_access_iterator_tag>::value, void>::type
        ...
        if (std::distance(inputBegin, inputEnd) <= sizeLimit)
            op(exec, make_iterator<InputIterator>()(inputEnd), make_iterator<InputIterator>()(inputBegin),
            make_iterator<OutputIterator>()(outputEnd), make_iterator<OutputIterator>()(outputBegin), std::forward<Rest>(rest)...);
    }
...
struct NonConstAdapter {
    ...
    auto operator()(Types&&... args) -> decltype(std::declval<F>().operator()(std::forward<Types>(args)...)) {
        return my_f(std::forward<Types>(args)...);
    }
};

template<typename F>
NonConstAdapter<F> non_const(const F& f) { return NonConstAdapter<F>(f); }
...
public:
    ...
    Wrapper& operator=(Wrapper&& input) {
        my_field = input.my_field;
        input.my_field = nullptr;
        ++move_count;
        return *this;
    }
    bool operator==(const Wrapper& input) const {
    ...
    friend std::ostream& operator<<(std::ostream& stream, const Wrapper& input) {
    ...
    T* get_my_field() const { return my_field.get(); };
    static size_t Count() { return my_count; }
    static size_t MoveCount() { return move_count; }
    static void SetCount(const size_t& n) { my_count = n; }
    static void SetMoveCount(const size_t& n) { move_count = n; }
private:
